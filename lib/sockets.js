...
Context.prototype.queueInfo = function(queue) {
...
function JobSocket(channel, opts) {
  Readable.call(this, {objectMode: true});
  Socket.call(this, channel, opts);
  this.consumers = {};
}
inherits(JobSocket, Readable);
addSocketMethods(JobSocket);

JobSocket.prototype.connect = function(queue, exchange, routingKey, callback) {
  var self = this, ch = this.ch;
  ...
  // Support the general form of connect
  if (callback === undefined && typeof routingKey === 'function') {
    callback = routingKey; routingKey = '';
  } else if (routingKey === undefined && typeof exchange === 'function') {
    callback = exchange; routingKey = ''; exchange = '';
  } else routingKey = routingKey || '';

  if (exchange) {
    ch.assertExchange(exchange, this.options.routing || 'fanout', {durable: self.options.durable || true, autoDelete: self.options.autoDelete})

      .then(function(ok) {
        return ch.assertQueue(queue, {durable: self.options.persistent});
      })
      .then(function(ok) {
      ...
      .then(function(ok) {
        return ch.consume(queue, function(msg) {
          ...
          self.consumers[queue] = ok.consumerTag;
        });
      })
      .then(callback);
  } else {
     ch.assertQueue(queue, {durable: self.options.persistent})
      .then(function(ok) {
        return ch.consume(queue, function(msg) {
          self.push(msg);
        }, {noAck:false}).then(function(ok) {
...
JobSocket.prototype.next = function(msg, socket, nextMsg, routingKey, encoding) {
  var rawMsg = typeof nextMsg == "object" ? JSON.stringify(nextMsg) : nextMsg;
  var self = this;
  //Push to next worker
  function send() {
    if (routingKey && socket.publish) {
      if (socket.publish(routingKey, rawMsg, encoding)){
        //Now ack the original message
        self.ack(msg);
      }
    } else {
      if (socket.write(rawMsg, encoding)){
        //Now ack the original message
        self.ack(msg);
      }
    }
...
JobSocket.prototype.ack = function(msg) {
  if (!msg) {
    throw new Error("ack[job] called with no unacknowledged messages");
  }
  this.ch.ack(msg);
};

JobSocket.prototype.nack = function(msg) {
  if (!msg) {
    throw new Error("nack[job] called with no unacknowledged messages");
  }
  this.ch.nack(msg);
};

JobSocket.prototype.requeue = function(msg) {
  if (!msg) {
    throw new Error("requeue[job] called with no unacknowledged messages");
  }
  this.ch.reject(msg);
};

//Allows for dead lettering
JobSocket.prototype.discard = function(msg) {
  if (!msg) {
    throw new Error("discard[job] called with no unacknowledged messages");
  }

  this.ch.reject(msg, false);
};

JobSocket.prototype._read = ignore;



function SendSocket(channel, opts) {
  Writable.call(this);
  Socket.call(this, channel, opts);
  this.pubs = [];
}
inherits(SendSocket, Writable);
addSocketMethods(SendSocket);
SendSocket.prototype.end = end;

SendSocket.prototype.connect = function(destination, callback) {
  var self = this, ch = this.ch;
  ch.assertExchange(destination, self.options.routing || 'fanout', {durable: self.options.durable || true, autoDelete: self.options.autoDelete})
    .then(function(ok) {
      self.pubs.push(destination);
    }).then(callback);
};

SendSocket.prototype.publish = function(topic, chunk, encoding) {
  var ch = this.ch, self = this;
  if (!topic) topic = this.options.topic || '';
  var options = {expiration: self.options.expiration,
...
SendSocket.prototype.write = function(chunk, encoding) {
  return this.publish(false, chunk, encoding);
};
